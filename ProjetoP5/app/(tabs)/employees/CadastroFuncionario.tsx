  ...
  return (
    <View style={styles.section}>
      ...
      <View style={styles.typeSelector}>
        ...
        >
          <Text style={[styles.typeButtonText, tipo === 'vendedor' && styles.typeButtonTextSelected]}>Vendedor</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.typeButton, tipo === 'representante' && styles.typeButtonSelected]}
          onPress={() => setTipo('representante')}
        >
          <Text style={[styles.typeButtonText, tipo === 'representante' && styles.typeButtonTextSelected]}>Representante</Text>
        </TouchableOpacity>
      </View>

      <Text style={styles.label}>Matrícula</Text>
      <TextInput value={matricula} onChangeText={setMatricula} style={styles.input} placeholder="Ex: VEN001" />

      <Text style={styles.label}>RG</Text>
      <TextInput value={rg} onChangeText={setRg} style={styles.input} placeholder="Ex: 12.345.678-9" />
      ...
      <TextInput value={salario} onChangeText={setSalario} style={styles.input} placeholder="Ex: 2500.00" keyboardType="decimal-pad" />

      {tipo === 'representante' && (
        <>
          <Text style={styles.label}>Data início contrato</Text>
          <TextInput value={dataInicioContrato} onChangeText={setDataInicioContrato} style={styles.input} placeholder="Ex: 2024-01-01" />

          <Text style={styles.label}>Data fim contrato</Text>
          <TextInput value={dataFimContrato} onChangeText={setDataFimContrato} style={styles.input} placeholder="Ex: 2026-12-31" />
          ...
          {categories.length === 0 ? (
            ...
            <View style={styles.categoryList}>
              {categories.map((cat) => (
                ...
                >
                  <Text style={[styles.categoryButtonText, categoriasResponsaveis.includes(cat.id) && styles.categoryButtonTextSelected]}>
                    {cat.codigo}
                  </Text>
                </TouchableOpacity>
              ))}
      ...
      {tipo === 'vendedor' && (
        <>
          ...
          <TextInput value={percentualComissao} onChangeText={setPercentualComissao} style={styles.input} placeholder="Ex: 5" keyboardType="decimal-pad" />

          <Text style={styles.label}>Código do cartão</Text>
          <TextInput value={codigoCartao} onChangeText={setCodigoCartao} style={styles.input} placeholder="Ex: CARD001" />

          <Text style={styles.label}>Data última progressão</Text>
          <TextInput value={dataUltimaProgressao} onChangeText={setDataUltimaProgressao} style={styles.input} placeholder="Ex: 2024-06-01" />

          <Text style={styles.label}>Categoria do cartão</Text>
          <View style={styles.categorySelector}>
            <TouchableOpacity style={[styles.categoryOption, categoriaCartao === 'Junior' && styles.categoryOptionSelected]} onPress={() => setCategoriaCartao('Junior')}>
              <Text style={[styles.categoryOptionText, categoriaCartao === 'Junior' && styles.categoryOptionTextSelected]}>Junior</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.categoryOption, categoriaCartao === 'Senior' && styles.categoryOptionSelected]} onPress={() => setCategoriaCartao('Senior')}>
              <Text style={[styles.categoryOptionText, categoriaCartao === 'Senior' && styles.categoryOptionTextSelected]}>Senior</Text>
            </TouchableOpacity>
          </View>
        </>
      )}

      <StandardButton text="Salvar funcionário" onPress={handleAddEmployee} variant="primary" />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  typeButtonText: { color: '#4a4a4a', fontWeight: '600' },
  typeButtonTextSelected: { color: '#fff' },
  categoryList: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  categoryButton: { paddingVertical: 8, paddingHorizontal: 10, borderRadius: 999, borderWidth: 1, borderColor: '#d6d6d6', backgroundColor: '#fff' },
  categoryButtonSelected: { backgroundColor: '#2c5aa0', borderColor: '#2c5aa0' },
  categoryButtonText: { color: '#4a4a4a', fontWeight: '600' },
  categoryButtonTextSelected: { color: '#fff' },
  categorySelector: { flexDirection: 'row', gap: 12 },
  categoryOption: { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: '#d6d6d6', backgroundColor: '#fff', alignItems: 'center' },
  categoryOptionSelected: { backgroundColor: '#2c5aa0', borderColor: '#2c5aa0' },
  categoryOptionText: { color: '#4a4a4a', fontWeight: '600' },
